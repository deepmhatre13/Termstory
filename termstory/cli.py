...
from termstory.formatter import (
    ...
    format_necromancer_score,
    format_rage_quit_signatures,
)
import sqlite3
...
def get_ai_provider_settings(config: dict) -> tuple[str, str, str, str]:
    ...
    api_key = get_config_value(config, f"providers.{provider}.api_key") or config.get(
        f"{provider}_api_key", ""
    )
    api_base_url = get_config_value(
        config, f"providers.{provider}.api_base_url"
    ) or config.get(f"{provider}_api_base_url", "")
    model_name = get_config_value(
        config, f"providers.{provider}.model_name"
...
def run_ingestion(db: Database) -> None:
    ...
    commands = parse_all_histories(
        history_files, db=db, project_paths=discover_project_paths()
    )
...
def search_history(
    query: Optional[str] = typer.Argument(
        None, help="Search term/query across commits, commands, and project names"
    ),
    project: Optional[str] = typer.Option(
        None, "--project", help="Filter matches by project name"
    ),
    since: Optional[str] = typer.Option(
        None, "--since", help="Filter matches since date YYYY-MM-DD"
    ),
    until: Optional[str] = typer.Option(
        None, "--until", help="Filter matches until date YYYY-MM-DD"
    ),
    tag: Optional[List[str]] = typer.Option(
        None,
        "--tag",
        "-t",
        help="Filter matches by tag(s) (deploy, debug, setup, test, docs)",
    ),
    limit: int = typer.Option(
        50, "--limit", help="Maximum number of search results to return"
    ),
    detailed: bool = typer.Option(
        False, "--detailed", help="Show all commands and commits in matched sessions"
    ),
    semantic: bool = typer.Option(
        False, "--semantic", help="Perform local semantic/RAG hybrid search"
    ),
    fts: bool = typer.Option(
        False, "--fts", help="Use SQLite FTS5 virtual tables for searching"
    ),
):
    ...
    if since:
        try:
            ...
            Console(stderr=True).print(
                f"[bold red]Error: Invalid date '{since}'. Expected YYYY-MM-DD.[/]"
            )
    ...
    if until:
        try:
            ...
            Console(stderr=True).print(
                f"[bold red]Error: Invalid date '{until}'. Expected YYYY-MM-DD.[/]"
            )
            raise typer.Exit(code=1)
    ...
    if tag:
        ...
        for t in tag:
            ...
            if t_clean not in TAGS:
                Console(stderr=True).print(
                    f"[bold red]Error: Invalid tag '{t_clean}'.[/bold red] Valid tags: {', '.join(TAGS)}."
                )
                raise typer.Exit(code=1)
    ...
    if semantic:
        if not query:
            Console(stderr=True).print(
                "[bold red]Error: Semantic search requires a search query.[/bold red]"
            )
            raise typer.Exit(code=1)
        try:
...
def show_today(
    compare: bool = typer.Option(
        False,
        "--compare/--no-compare",
        help="Compare each project's time with yesterday",
    ),
):
...
def show_project(
    name: str = typer.Argument(
        ..., help="Name or path of the project, or 'context' to manage project context"
    ),
    arg2: Optional[str] = typer.Argument(
        None, help="Project name (required if first argument is 'context')"
    ),
    arg3: Optional[str] = typer.Argument(
        None, help="Context description to set (if setting context)"
    ),
    ...
    if name.lower() == "context":
        ...
        target = None
        for p in projects:
            if arg2.lower() in p.name.lower() or (
                p.path and arg2.lower() in p.path.lower()
            ):
                target = p
                break
        ...
        if not target:
            Console(stderr=True).print(
                f"[bold red]Error: Could not find project matching '{arg2}'[/]"
            )
            raise typer.Exit(code=1)
        ...
        if show:
            ...
            console.print(
                f"[bold green]✅ Context updated for project '{target.name}':[/] {arg3}"
            )
        return
    ...
    target = None
    for p in projects:
        if name.lower() in p.name.lower() or (
            p.path and name.lower() in p.path.lower()
        ):
            target = p
            break
    ...
    if not target:
        Console(stderr=True).print(
            f"[bold red]Error: Could not find project matching '{name}'[/]"
        )
        raise typer.Exit(code=1)
...
def show_tags(
    tag: Optional[str] = typer.Argument(
        None,
        help="Filter and list sessions by a specific tag (deploy, debug, setup, test, docs)",
    ),
    rebuild: bool = typer.Option(
        False, "--rebuild", "-r", help="Force rebuild/re-evaluate tags for all sessions"
    ),
    ...
    if not tag:
        ...
        for t in TAGS:
            ...
            dur = tag_durations[t]
            output_lines.append(
                f"{t:<8} : {count:>3} sessions ({format_duration(dur)})"
            )
        ...
        if tag not in TAGS:
            console.print(
                f"[bold red]Error: Invalid tag '{tag}'.[/bold red] Valid tags: {', '.join(TAGS)}."
            )
            raise typer.Exit(1)
        ...
        for row in session_rows:
            ...
            if ai_sum:
                ...
                cursor.execute(
                    "SELECT command FROM commands WHERE session_id = ? ORDER BY timestamp ASC LIMIT 1",
                    (s_id,),
                )
...
def show_web(
    template: Optional[str] = typer.Option(
        None, "--template", help="Template name or custom HTML template file path"
    ),
    date_range: Optional[str] = typer.Option(
        None,
        "--date-range",
        help="Date range filter (e.g. today, yesterday, 7days, 30days, YYYY-MM-DD:YYYY-MM-DD)",
    ),
):
...
def ask_cmd(
    query: str = typer.Argument(
        ..., help="Question to ask about your development history"
    ),
):
    ...
    config = load_config()
    with console.status(
        "[bold green]Analyzing history and generating answer...[/bold green]"
    ):
...
def predict_cmd(
    top: int = typer.Option(
        3, "--top", help="Number of top project predictions to show"
    ),
    json_out: bool = typer.Option(False, "--json", help="Output predictions as JSON"),
    days: Optional[int] = typer.Option(
        None, "--days", help="Number of days of history to analyze"
    ),
):
...
def anger_translator(
    ...
    limit: int = typer.Option(
        5, "--limit", help="Maximum number of commits to analyze"
    ),
):
    """
    ...
    try:
        ...
        if project:
            cursor.execute(
                "SELECT id FROM projects WHERE name LIKE ?", (f"%{project}%",)
            )
            row = cursor.fetchone()
            if row:
                project_id = row[0]
                ...
                Console(stderr=True).print(
                    f"[bold red]Error: Project '{project}' not found.[/]"
                )
                raise typer.Exit(code=1)
    ...
    if ai_enabled and provider != "disabled":
        ...
        console.print(
            "[bold yellow]Translating developer's git blame and terminal frustration...[/]"
        )
...
def fortune_teller(
    limit: int = typer.Option(
        5, "--limit", help="Maximum number of chaotic sessions to analyze"
    ),
):
    """
    ...
    if ai_enabled and provider != "disabled":
        ...
        console.print(
            "[bold yellow]Foretelling potential bugs from late-night chaotic session telemetry...[/]"
        )
...
def cleanup_shell_marker():
    ...
    old_block_pattern_zsh = re.compile(
        r"\n?# TermStory Timekeeping\nsetopt EXTENDED_HISTORY\n?"
    )
    old_block_pattern_bash = re.compile(
...
def perform_reset(auto_confirm: bool = False, dry_run: bool = False):
    ...
    console.print(
        "[bold red]WARNING: The following paths will be permanently deleted:[/bold red]"
    )
    ...
    if not auto_confirm:
        ...
        if not sys.stdin.isatty():
            console.print(
                "[bold red]Refusing to reset in a non-interactive shell without --yes.[/bold red]"
            )
            raise typer.Exit(code=1)
        try:
            response = (
                input("\nAre you sure you want to delete all TermStory data? (y/n): ")
                .strip()
                .lower()
            )
...
def show_ui(
    ...
    all_history: bool = typer.Option(
        False, "--all", help="Display all recorded history"
    ),
):
    ...
    if missing_ts and not _cfg.get("has_seen_timestamp_prompt", False):
        ...
        if response in ("y", "yes"):
            try:
                ...
                if os.path.exists(config_path):
                    with open(config_path, "r") as f:
                        content = f.read()
                    if (is_bash and "HISTTIMEFORMAT" in content) or (
                        not is_bash and "EXTENDED_HISTORY" in content
                    ):
                        already_exists = True
                ...
                console.print(
                    f"[bold red]Error modifying {config_display}: {e}[/bold red]"
                )
            ...
            console.print(
                "Invalid response. Continuing with legacy history fallback..."
            )
    ...
    if getattr(app_tui, "was_reset", False):
        ...
        try:
            ...
            if (
                ...
                console.print(
                    "To configure a local or cloud AI provider (Groq, OpenAI, Ollama), run:"
                )
                console.print(
                    "  [cyan]termstory config set active_provider groq[/cyan] (or [cyan]openai[/cyan] / [cyan]ollama[/cyan])"
                )
                console.print(
                    "  [cyan]termstory config set providers.groq.api_key <your_api_key>[/cyan]"
                )
                console.print(
                    "Alternatively, press [bold]? [/bold]inside the TUI to open the onboarding settings anytime.\n"
                )
                _cfg["has_seen_onboarding_reminder"] = True
                save_config(_cfg)
        except Exception as e:
            Console(stderr=True).print(
                f"[dim]Note: failed to persist onboarding reminder flag: {e}[/dim]"
            )
...
def run_agy():
    ...
    if not agy_path:
        Console(stderr=True).print(
            "[bold red]Error: 'agy' command not found on PATH.[/]"
        )
        raise typer.Exit(code=1)
...
def replay_cmd(
    session_id: Optional[int] = typer.Argument(
        None,
        help="ID of the session to replay. If not provided, the most recent session is used.",
    ),
    speed: float = typer.Option(
        1.0,
        "--speed",
        "-s",
        help="Playback speed multiplier (e.g. 2.0 for fast, 0.5 for slow)",
    ),
    list_sessions: bool = typer.Option(
        False, "--list", "-l", help="List recent sessions to choose from"
    ),
    mcp: bool = typer.Option(
        False, "--mcp", help="Show captured MCP snapshots for the session"
    ),
):
    ...
    if mcp:
        ...
        if not sessions:
            Console().print(
                f"[bold red]Error: Session #{session_id} not found.[/bold red]"
            )
            raise typer.Exit(code=1)
...
def export_cmd(
    ...
    project: Optional[str] = typer.Option(
        None, "--project", "-p", help="Filter by project name or path"
    ),
    since: Optional[str] = typer.Option(
        None,
        "--since",
        "-s",
        help="Filter by since duration (e.g. '7' for 7 days, or YYYY-MM-DD)",
    ),
):
    ...
    if not sessions:
        Console(stderr=True).print(
            "[yellow]No sessions found matching filters.[/yellow]"
        )
    ...
    if fmt == "json":
        ...
        Console(stderr=True).print(
            f"[bold red]Error: Unsupported format '{format}'. Use 'json' or 'csv'.[/]"
        )
        raise typer.Exit(code=1)
...
def archive_cmd(
    days: int = typer.Option(
        90, "--days", "-d", help="Archive sessions older than N days"
    ),
    archive_db: Optional[str] = typer.Option(
        None,
        "--archive-db",
        "-a",
        help="Path to the archive SQLite database file (defaults to archive.db next to the main database)",
    ),
):
    ...
    try:
        ...
        console.print(
            f"  Macro summaries archived: [bold]{stats['macro_summaries']}[/]"
        )
    except Exception as e:
...
def timeline_cmd(
    days: int = typer.Option(
        30, "--days", help="Number of days to include in the timeline"
    ),
) -> None:
...
def notebook_cmd(
    ...
    all_commands: bool = typer.Option(
        False,
        "--all-commands",
        help="Include all commands (including navigation/utility noise)",
    ),
    reverse: bool = typer.Option(
        False, "--reverse", help="Sort days reverse-chronologically (latest first)"
    ),
):
    ...
    markdown_content = generate_notebook(
        sessions, db, all_commands=all_commands, reverse=reverse
    )
    ...
    if output and output != "-":
        ...
        console.print(
            f"[bold green]✅ Notebook successfully exported to {escape(output)}[/]"
        )
    else:
...
def config_set(key: str, value: str):
    ...
    if isinstance(current_val, bool):
        ...
        if (
            key in ("ai_enabled", "has_seen_onboarding")
            or key.endswith(".ai_enabled")
            or key.endswith(".has_seen_onboarding")
        ):
            converted_value = value.lower() in ("true", "1", "yes")
    ...
    console.print(
        f"[green]Set config key '{escape(key)}' to '{escape(str(set_val))}'[/]"
    )
...
def remind_cmd(
    ...
    list_all: bool = typer.Option(
        False, "--list", "-l", help="List all active/pending reminders"
    ),
    complete: Optional[int] = typer.Option(
        None, "--complete", "-c", help="Complete a reminder by its ID"
    ),
    show_completed: bool = typer.Option(
        False, "--show-completed", "-a", help="Show completed reminders when listing"
    ),
):
    ...
    if complete is not None:
        ...
        if success:
            console.print(
                f"[bold green]✅ Marked reminder #{complete} as completed![/]"
            )
        else:
    ...
    filtered = (
        reminders
        if show_completed
        else [r for r in reminders if r.get("status") == "pending"]
    )
...
def show_profile(
    limit: int = typer.Option(
        ...
        "--limit",
        "-l",
        help="Maximum number of queries to display in each category",
    ),
):
...
def main(
    ...
    date: Optional[str] = typer.Option(
        None,
        "--date",
        help="Date override (e.g. today, yesterday, 7days, YYYY-MM-DD) for commands",
    ),
    reset: bool = typer.Option(
        False, "--reset", help="Reset all TermStory state, configuration, and database"
    ),
...
def show_rpg_class():
    ...
    if ai_enabled and provider != "disabled":
        ...
        console.print(
            "[bold yellow]Querying LLM to generate your custom RPG developer biography...[/]"
        )
...
def show_rage_quit():
    ...
    stats = analyze_all(db)
    rage_quit_info = stats.get(
        "rage_quit_info", {"signatures": [], "events": [], "total_events": 0}
    )
...
def sleep_cmd(
    ...
    show: bool = typer.Option(False, "--show", help="View all consolidated contexts"),
):
    ...
    if consolidate:
        ...
        console.print(
            f"[bold green]✅ Consolidation complete. Created {count} new consolidated contexts.[/bold green]"
        )
        raise typer.Exit()
    ...
    if show:
        ...
        table = Table(
            title="💤 REM Sleep Consolidated Contexts", box=SIMPLE, border_style="cyan"
        )
        ...
        for c in contexts:
            start_dt = datetime.fromtimestamp(c["start_time"]).strftime(
                "%Y-%m-%d %H:%M:%S"
            )
