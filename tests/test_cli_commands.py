    ...
    commits = [
        {
            ...
            "timestamp": now,
            "message": "feat: Add docker health check",
            "cleaned_message": "Add docker health check",
        }
    ]
...
def test_cli_config_commands(tmp_path, monkeypatch):
    ...
    result = runner.invoke(
        ...
        [
            ...
            "sk-proj-test-openai-key-abc-123",
        ],
    )
    assert result.exit_code == 0
    ...
    assert (
        "test-g...-123" in result.stdout
    )  # Redacted key should be printed in redacted format
...
def test_cli_ui_onboarding_missing_timestamps_yes(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "os.path.expanduser", lambda p: str(zshrc_file) if p == "~/.zshrc" else p
    )
...
def test_cli_ui_onboarding_reminder_suppressed_if_seen_onboarding(
...
def test_cli_reset_cleanup_rc_files(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "os.path.expanduser", lambda p: str(tmp_path / os.path.basename(p))
    )
...
def test_cli_error_states(tmp_path, monkeypatch):
    ...
    assert result.exit_code == 1
    assert (
        "Could not find project matching" in result.stdout
        or "Could not find project matching" in result.stderr
    )
    ...
    assert result.exit_code == 1
    assert (
        "Config key 'some.missing.key' not found" in result.stdout
        or "Config key 'some.missing.key' not found" in result.stderr
    )
    ...
    assert result.exit_code == 1
    assert (
        "Invalid date format" in result.stdout or "Invalid date format" in result.stderr
    )
...
def test_global_date_accepts_natural_language(tmp_path, monkeypatch):
    ...
    assert (
        os.environ.get("TERMSTORY_DATE_OVERRIDE") == expected
...
def test_cli_agy_command(monkeypatch):
    ...
    monkeypatch.setattr(
        shutil, "which", lambda cmd: "/usr/local/bin/agy" if cmd == "agy" else None
    )
    ...
    assert (
        "Error: 'agy' command not found" in result2.stdout
        or "Error: 'agy' command not found" in result2.stderr
    )
...
def test_cli_project_context_command(tmp_path, monkeypatch):
    ...
    # 1. Set context
    result = runner.invoke(
        app, ["project", "context", "HugeGraph", "This project is for graph databases"]
    )
    assert result.exit_code == 0
...
def test_cli_today_command_no_compare(tmp_path, monkeypatch):
    ...
    # Use current time so sessions fall within today's range automatically
    from termstory.date_utils import get_current_time
    ...
    assert result.exit_code == 0
    assert "Apache HugeGraph" in result.stdout
    assert "vs yesterday" not in result.stdout


def test_cli_today_command_compare(tmp_path, monkeypatch):
    ...
    # Use current time for today, and yesterday for the compare session
    from termstory.date_utils import get_current_time
    ...
    s_yesterday = Session(
        ...
        end_time=yesterday + 50,
        duration_seconds=50,
        project_id=1,
        commands=[cmd],
    )
    db.save_data([p], [s_today], [cmd])
    db.save_data([p], [s_yesterday], [cmd])

    runner = CliRunner()
    result = runner.invoke(app, ["today", "--compare"])
    assert result.exit_code == 0
    assert "Apache HugeGraph" in result.stdout
    assert "vs yesterday" in result.stdout


def test_cli_today_command_no_compare_flag(tmp_path, monkeypatch):
